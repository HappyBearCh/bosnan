...
module.exports = [
  ...
  },
  {
    ...
    creator: 'Toshio Kai',
    sfxType: 'Action loop',
    description: 'The chattering two-note loop that plays as Pac-Man eats dots was built by Namco sound designer Toshio Kai from rapidly alternating square waves — an imitation of a mouth working, assembled from the only materials a three-channel sound chip could offer.',
    longDescription: 'Toshio Kai composed Pac-Man\'s music and designed its sound effects in 1980, working on Namco\'s custom arcade hardware: an 8-bit CPU and a sound chip with three channels. There was no capacity for a recorded sample of chewing, and no synthesiser subtle enough to imitate one. Kai\'s solution was to alternate rapidly between two square waves at different pitches, producing a fast, repeating two-note chatter that the ear readily interprets as a mouth opening and closing. The effect loops continuously for as long as the player is consuming dots, which means it is not a discrete event sound at all but a state indicator — it tells you, without your having to look, that you are still eating, still scoring, still moving productively through the maze.\n\nThat continuous quality is why the sound became the game\'s signature. A Pac-Man cabinet in a crowded arcade announced itself across the room by the waka-waka, and the sound entered popular culture under a name no designer chose: players heard "waka waka" in the alternation and the onomatopoeia stuck permanently. Kai arrived at one of the most recognisable sounds in entertainment history not through sophistication but through a precise reading of what two square waves, switched quickly enough, could persuade a listener to hear.',
    keyFacts: [
  ...
  },
  {
    ...
    sections: [
      {
        title: 'Engineering Anticipation',
        html: '<p>Fujisawa\'s brief was not to make a pleasant noise but to produce a specific emotional state, and the tuning choice is where that intent becomes visible. Equal temperament is the harmonic environment nearly all Western listeners live in; it is the sound of stability and familiarity. Pure intonation, with its exact frequency ratios, sits slightly outside that expectation. By starting in one and drifting toward the other, the chime establishes solid ground and then quietly removes it — the listener registers something has shifted without being able to identify what.</p><p>That unresolved quality is anticipation, mechanically produced. A sound that simply resolved would signal completion; the PlayStation chime signals that something is about to begin. Fujisawa\'s cinema analogy is exact: the feeling he was after is the one you get when the lights go down and before anything has happened, and he built it out of tuning systems rather than leaving it to instinct.</p>',
      },
      {
        title: 'A Chime That Diagnoses',
        html: '<p>Underneath the artistry sits a piece of systems design. The startup sound is a status report — it plays when the hardware has confirmed it is operating correctly and the disc has been read successfully, which means a player who hears it in full knows, without any text on screen, that everything is fine. The swooshing reverse element is engineered to loop when the disc cannot be read, so a failure state is communicated by the same asset that communicates success, simply behaving differently.</p><p>This dual role is why the sound occupies such a large place in the memory of PlayStation owners. It was not decoration attached to a boot sequence; it was the boot sequence, the machine reporting on itself in a language the player learned without being taught. Hearing it meant the disc had taken, the console was healthy, and play was seconds away — a Pavlovian association built over thousands of repetitions across the most successful console of its generation.</p>',
      },
    ],
  },
];
