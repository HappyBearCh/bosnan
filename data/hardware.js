...
module.exports = [
  ...
  },
  {
    id: 'atari-tia',
    name: 'TIA',
    fullName: 'Television Interface Adaptor',
    manufacturer: 'Atari',
    year: 1977,
    usedIn: 'Atari 2600 (VCS)',
    description: 'A graphics chip with no frame buffer — it holds a single scanline at a time, forcing the CPU to rewrite its registers in the gap between lines. Programmers called it "racing the beam".',
    longDescription: 'The TIA, designed by a team led by Jay Miner, is the chip that made the Atari 2600 cheap enough to exist and hellish enough to program. RAM in 1977 was expensive, so Atari simply did not include enough of it to hold a screen. The TIA has registers for exactly one scanline of display — no frame buffer, no screen memory, nothing to compose an image in. What appears on the television is generated line by line, in real time, as the electron beam sweeps across the tube.\n\nThe consequence is one of the strangest programming models in computing history. The 6502 loads the TIA\'s registers with the data for the first line. The TIA waits for the television to be ready, reads the registers, and emits the signal for that line. Then, during the horizontal blanking interval — the brief moment while the beam retraces to the left edge — the CPU must rewrite every register it needs to change before the next line is drawn. Miss the window and the picture is wrong. There is no way to fall behind and catch up; the television does not wait. Every 2600 game is therefore a program written in lockstep with a cathode ray tube, and the entire visual repertoire of the console — the mid-screen colour changes, the impossible-looking effects, the flickering when too many objects share a line — is a direct consequence of what a 6502 can and cannot do in the time it takes a beam to travel back across a screen.\n\nMiner went on to expand the approach into the ANTIC, CTIA/GTIA and POKEY chips for the Atari 8-bit computers, and later to lead the design of the Amiga\'s custom chipset.',
    specs: [
      { label: 'Frame buffer', value: 'None — one scanline of registers' },
      { label: 'Sprites', value: '2 players, 2 missiles, 1 ball' },
      { label: 'Playfield', value: '20 bits, mirrored or repeated across the screen' },
      { label: 'Audio', value: '2 channels, 4-bit volume' },
      { label: 'CPU coupling', value: 'Registers rewritten during horizontal blank ("racing the beam")' },
    ],
  },
  {
    id: 'amiga-ocs',
    name: 'Agnus / Denise / Paula',
    fullName: 'Amiga Original Chip Set (OCS)',
    manufacturer: 'MOS Technology (Commodore)',
    year: 1985,
    usedIn: 'Amiga 1000, 500, 2000',
    description: 'Three custom chips with names like a family, and a coprocessor that executes an instruction stream synchronised to the electron beam — the reason Amiga demos looked impossible.',
    longDescription: 'The Amiga\'s Original Chip Set was conceived by Jay Miner — the same engineer behind the Atari 2600\'s TIA — and manufactured by Commodore\'s subsidiary MOS Technology. Its power comes from a design philosophy that was genuinely unusual in 1985: rather than making the CPU do everything, offload as much as possible onto dedicated silicon, and let the 68000 supervise.\n\nAgnus is the centre of the machine. It arbitrates all access to chip RAM between the 68000 and the other custom chips through a priority system, and it contains two sub-processors that do the actual work. The Blitter moves and combines blocks of memory at high speed without troubling the CPU, which is why Amiga games could scroll and animate large areas while the processor was busy elsewhere. The Copper — short for co-processor — is the more remarkable of the two: a programmable state machine that executes its own instruction stream, synchronised to the video beam. It can change hardware registers at precise screen positions, mid-frame, which is how the Amiga produces its signature effects — gradient skies, split screens with different resolutions, colour palettes that change halfway down the display.\n\nDenise handles the graphics output proper: bitplane display, palette registers, smooth scrolling, hardware sprites, screen object priority and collision detection. Paula is the audio chip, with four independent hardware-mixed 8-bit PCM channels, each with 65 volume levels and sample rates from about 20 Hz up to nearly 29 kHz — and, in one of the era\'s stranger design decisions, Paula also handles interrupts, the serial port, analogue joysticks and the floppy drive.',
    specs: [
      { label: 'Agnus', value: 'Memory arbitration, Blitter, Copper' },
      { label: 'Denise', value: 'Bitplanes, palette, hardware sprites, collision detection' },
      { label: 'Paula', value: '4-channel 8-bit PCM audio, interrupts, floppy, serial, joystick' },
      { label: 'Copper', value: 'Beam-synchronised coprocessor with its own instruction stream' },
      { label: 'Audio', value: '4 channels, 65 volume levels, up to ~29 kHz' },
    ],
  },
  {
    id: 'ps2-emotion-engine',
    name: 'Emotion Engine',
    fullName: 'Emotion Engine (EE)',
    manufacturer: 'Sony Computer Entertainment / Toshiba',
    year: 2000,
    usedIn: 'PlayStation 2',
    description: 'Six instruction pipelines across three processors, manual DMA, hand-written vector microcode, and no compiler on earth able to exploit it. The most powerful console CPU of its generation and the most hated.',
    longDescription: 'The Emotion Engine was developed jointly by Sony Computer Entertainment and Toshiba, and it is the reason the PlayStation 2 was simultaneously the best-selling console ever made and a byword for developer misery. It is not a single processor but a cluster: a MIPS-derived core, a floating-point unit, and — critically — two Vector Processing Units, VPU0 and VPU1. Each VPU is a small independent processor built to operate on vectors of four floats, executing an operation per cycle, and they exist because 3D geometry is fundamentally vector maths.\n\nExtracting that performance was the problem. Developers had to manage DMA transfers by hand, schedule parallel workloads explicitly, and write custom microcode for the vector units. As one developer put it, the main processor and the two vector units each have two instruction pipelines, and the only way to approach optimal results is to hand-write assembly that micro-manages all six pipelines while also exploiting the instruction and data caches — and no C compiler has ever been written that can handle even a fraction of that complexity. In practice, teams wrote the main CPU code in C or C++ and dropped into assembly for anything performance-critical.\n\nThis is precisely why Criterion\'s RenderWare middleware became so dominant on the platform and was nicknamed "Sony\'s DirectX". The Emotion Engine was extraordinarily fast for anyone willing to hand-optimise it in assembly, and most studios were not — so they licensed an engine from someone who had.',
    specs: [
      { label: 'Core', value: 'MIPS-derived CPU @ 294.912 MHz' },
      { label: 'Vector units', value: 'VPU0 and VPU1 — 4-float vector operations, 1 per cycle' },
      { label: 'Pipelines', value: '6 total across CPU and both vector units' },
      { label: 'Programming', value: 'Manual DMA, hand-written vector microcode, assembly for hot paths' },
      { label: 'Paired with', value: 'Graphics Synthesizer (GS)' },
    ],
  },
  {
    id: 'konami-vrc6',
    name: 'Konami VRC6',
    fullName: 'VRC6 (Konami mapper with expansion audio)',
    manufacturer: 'Konami',
    year: 1990,
    usedIn: 'Akumajō Densetsu (Castlevania III, Famicom)',
    description: 'A Konami cartridge chip that added three extra sound channels to the Famicom, giving the Japanese Castlevania III a richer score than the Famicom could otherwise produce — audio the American NES version could not reproduce.',
    longDescription: 'The VRC6 was one of a family of custom cartridge chips (VRC, for "Konami Virtual ROM Controller") that Konami built for its Famicom games. Like other mappers it handled memory bank-switching to allow larger, more complex games, but its distinguishing feature was audio: the VRC6 added three extra sound channels on top of the Famicom\'s built-in five, and it was used most famously in Akumajō Densetsu, the Japanese version of Castlevania III: Dracula\'s Curse (1990).\n\nThe Famicom\'s native sound hardware provided five channels — two pulse (square) waves, a triangle wave, a noise channel, and a primitive sample channel. The VRC6 supplemented these with two additional pulse channels and, unusually, a sawtooth-wave channel, a waveform the base hardware could not generate at all. Those extra voices gave composers more room for harmony, counter-melody, and a distinctive brighter, buzzier timbre, and Akumajō Densetsu\'s celebrated soundtrack is the showcase: it simply has more going on, and more tonal variety, than the console alone could manage.\n\nThe reason this became a tale of two versions lies in a hardware difference between the Famicom and the NES. On the Japanese Famicom, the cartridge slot included pins that let a cartridge\'s own audio be mixed back into the console\'s sound output, so expansion chips like the VRC6 could add channels that played through the system. Nintendo routed audio differently on the North American NES — the cartridge could not pipe expansion sound to the console the same way — which meant the VRC6\'s extra channels could not be used on NES hardware.\n\nAs a result, the North American Castlevania III lost the VRC6 entirely, and its music had to be rebuilt using the console\'s standard capabilities, producing a noticeably thinner soundtrack than the Japanese original. For years this made Akumajō Densetsu\'s audio a prized example of what the Famicom could do with expansion hardware, and the VRC6 a favourite of chiptune musicians. It stands as one of the clearest illustrations that on the 8-bit Nintendo, some of the best-sounding games achieved it not through the console but through custom silicon hidden inside the cartridge.',
    specs: [
      { label: 'Type', value: 'Cartridge mapper with expansion audio' },
      { label: 'Extra channels', value: '2 pulse + 1 sawtooth (3 total)' },
      { label: 'Base Famicom audio', value: '5 channels (2 pulse, triangle, noise, sample)' },
      { label: 'Showcase title', value: 'Akumajō Densetsu / Castlevania III (1990)' },
      { label: 'NES limitation', value: 'Expansion audio unusable on North American NES hardware' },
    ],
  },
  {
    id: 'nintendo-dsp-1',
    name: 'Nintendo DSP-1',
    fullName: 'DSP-1 (NEC µPD77C25 signal processor)',
    manufacturer: 'Nintendo / NEC',
    year: 1990,
    usedIn: 'Pilotwings, Super Mario Kart (SNES)',
    description: 'A math coprocessor tucked into SNES cartridges to do the 3D calculations the console\'s CPU could not — the chip that made the pseudo-3D perspective of Pilotwings and Super Mario Kart run smoothly.',
    longDescription: 'The DSP-1 was the first of the Super Nintendo\'s in-cartridge enhancement chips, a digital signal processor added to specific game cartridges to perform mathematics the console\'s own CPU was too slow to handle in real time. It was based on the NEC µPD77C25, a 16-bit fixed-point signal processor originally designed for tasks like speech analysis, which Nintendo had reprogrammed with custom firmware full of the trigonometry and 3D transformation routines that pseudo-3D games required.\n\nThe SNES could already produce a striking pseudo-3D effect on its own through a graphics feature called Mode 7, which let a background layer be rotated and scaled to simulate a plane receding into the distance. But convincingly animating that plane — computing the perspective for a banking aircraft or a kart cresting a hill, frame after frame — demanded a heavy load of fast floating-point-style calculation that the SNES\'s main processor could not sustain. The DSP-1\'s job was to take on that math: the game would hand it the geometry, and the chip would return the transformed coordinates the SNES needed to draw the next frame.\n\nThe results were foundational to the console\'s identity. Pilotwings, a Japanese launch-window title in late 1990, was the very first game to use the coprocessor, using it to render its flight and skydiving sequences. Super Mario Kart (1992) is the more famous example, relying on the DSP-1 to compute the constantly shifting perspective of its tracks and the illusion of vehicles moving through a three-dimensional space. In both cases the visible Mode 7 spectacle was only possible because a hidden chip in the cartridge was doing the underlying arithmetic.\n\nThe DSP-1 became the most widely used of the SNES\'s many enhancement chips, appearing in more than fifteen games. It established a pattern that defined the SNES generation: rather than build every capability into the console, Nintendo let cartridges carry supplementary processors — the DSP series, and later the more powerful Super FX — so that individual games could exceed the base hardware\'s limits. It is a clean example of how much of the SNES\'s most impressive output depended on silicon the buyer never saw, purchased one cartridge at a time.',
    specs: [
      { label: 'Base chip', value: 'NEC µPD77C25, 16-bit fixed-point DSP' },
      { label: 'Purpose', value: '3D / perspective math for Mode 7 effects' },
      { label: 'First game', value: 'Pilotwings (1990)' },
      { label: 'Famous use', value: 'Super Mario Kart (1992)' },
      { label: 'Adoption', value: 'Most-used SNES enhancement chip — 15+ games' },
    ],
  },
  {
    id: 'atari-pokey',
    name: 'Atari POKEY',
    fullName: 'POKEY (Pot Keyboard Integrated Circuit)',
    manufacturer: 'Atari',
    year: 1979,
    usedIn: 'Atari 8-bit computers, Atari 5200, many arcade games',
    description: 'Atari\'s multi-purpose chip that generated four voices of sound while also reading paddle controllers and keyboards — the audio of the Atari 8-bit computers and of arcade classics like Centipede and Gauntlet.',
    longDescription: 'POKEY — short for "Pot Keyboard Integrated Circuit" — was designed by Doug Neubauer at Atari and first shipped in 1979 with the Atari 400 and 800 home computers, later appearing in every model in that family and in the Atari 5200 console. Its name reflects its curiously mixed job description: it was a single chip that combined several unrelated functions the system needed, from input handling to sound to random-number generation, an economical piece of design typical of the era\'s effort to do as much as possible with as few chips as possible.\n\nThe "Pot" and "Keyboard" in its name refer to two of those functions. POKEY read the potentiometers in paddle controllers — the analog dials that games like the era\'s bat-and-ball titles depended on — and it scanned the computer\'s keyboard matrix. It also served as a source of pseudorandom numbers, a genuinely useful facility for games that needed unpredictability. But the function it is best remembered for is audio: POKEY produced four independent voices of square-wave sound, playable as clean tones or reshaped with a set of distortion settings that gave it a gritty, characterful timbre. While Neubauer designed the chip overall, its audio components were the work of Steven Mayer and Ronald Milner.\n\nThat sound hardware defined the voice of the Atari 8-bit computers, and Neubauer himself demonstrated its potential in Star Raiders, the platform\'s landmark space-combat game and one of its killer applications. But POKEY\'s reach extended well beyond the home. Atari used the chip for audio in a long list of arcade games through the 1980s — Centipede, Missile Command, Asteroids Deluxe, and Gauntlet among them — so that the same silicon responsible for the sound of a home computer was also responsible for the sound of some of the most famous cabinets in the arcade.\n\nPOKEY endures as a favourite among chiptune musicians and a distinctive piece of Atari\'s technical heritage. Its four raspy voices are as recognisable to enthusiasts of the Atari sound as the SID is to Commodore fans, and its dual life across home and arcade hardware makes it one of the more versatile and widely heard chips of its generation. That a single component could read your paddle, scan your keyboard, roll dice, and sing was very much the point — and it sang well enough to be remembered for it four decades later.',
    specs: [
      { label: 'Designer', value: 'Doug Neubauer (audio by Steven Mayer & Ronald Milner)' },
      { label: 'Audio', value: '4 voices, square wave with distortion options' },
      { label: 'Also handles', value: 'Paddle pots, keyboard scan, pseudorandom numbers' },
      { label: 'Home use', value: 'Atari 400/800, Atari 5200' },
      { label: 'Arcade use', value: 'Centipede, Missile Command, Gauntlet, and more' },
    ],
  },
];
